...
;;;   AS4UPDATE         - re-import a file: removes objects previously
;;;                       imported from that same file, then re-imports it
...
;;;   - AS4IMPORT/AS4UPDATE write nothing to disk beyond the drawing
...
(defun AS4:trim (s) (AS4:ltrim (AS4:rtrim s)))
...
;; auto-detects the delimiter (tab, then semicolon, then comma - the first
;; one that splits the line into more than one field wins), collapses
;; repeated delimiter runs first, then trims every resulting field.
...
;; -----------------------------------------------------------------------
;; core import routine (called by AS4IMPORT and AS4UPDATE)
;; -----------------------------------------------------------------------
...
;; -----------------------------------------------------------------------
;; AS4UPDATE: remove objects previously imported from the same file
;; (matched via the "originfile" Xdata value), then re-import it. This
;; avoids duplicating geometry when re-running an updated/corrected CSV.
;; -----------------------------------------------------------------------
(defun AS4:xdata-matches-file (ent path / xd lst found pair)
  ...
  (if xd
    (progn
      ...
      (foreach pair lst
        (if (and (= (car pair) 1000) (= (cdr pair) (strcat "originfile=" path)))
          (setq found T)
        )
      )
    )
...
(defun c:AS4UPDATE ( / originfile ss i ent removed)
  (setq originfile (getfiled "Select ArchSurv text file to re-import" "" "csv;txt;asc" 4))
  (if (not originfile)
    (progn (princ "\nCancelled.") (princ) (exit))
  )
  (setq removed 0)
  (setq ss (ssget "X" (list (cons -3 (list (list "AS4QGIS"))))))
  (if ss
    (progn
      (setq i 0)
      (while (< i (sslength ss))
        (setq ent (ssname ss i))
        (if (and (entget ent) (AS4:xdata-matches-file ent originfile))
          (progn (entdel ent) (setq removed (1+ removed)))
        )
        (setq i (1+ i))
      )
    )
  )
  (princ (strcat "\n" (itoa removed) " previously imported object(s) for this file removed."))
  (AS4:run-import originfile)
)
              ...
              (foreach rowdata rows
                ...
                (cond
                  ...
                  ((= kind "line")
                    ...
                    ;; the mirrored polyline copy, same properties, an
                    ;; open LineString of the raw vertices (matching
                    ;; how this system already exports its own native
                    ;; polyline shptypes - no artificial ring-closing
                    ;; duplicate)
                    (if (and ispoly (= mirror "Yes"))
                      (setq mirrorjson (strcat "{\"type\":\"LineString\",\"coordinates\":" (AS4:json-coord-list pts) "}"))
                      (setq mirrorjson nil)
                    )
                  )
                )
...
(defun c:AS4INFO ( / cmds descs choice idx)
  (setq cmds (list
    ...
    "AS4SETTINGS" "AS4STATUS" "AS4TAGFROMLAYER" "AS4UPDATE"
    "AS4XPORT" "AS4ZOOM"
  ))
  (setq descs (list
    ...
    "Re-imports a file: removes objects previously imported from that same file, then imports it again. Usage: run it, select the (corrected) text file."
...
(princ (strcat "\nAS4CAD v" *AS4:VERSION* " loaded. Commands: AS4IMPORT, AS4UPDATE, AS4NET, AS4SELECT, AS4SETTINGS, AS4STATUS, AS4TAGFROMLAYER, AS4XPORT, AS4ZOOM, AS4INFO. Type AS4INFO for a full command list with descriptions."))
(princ)
